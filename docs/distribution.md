...
```sh
cargo install --git https://github.com/nvk/chattybara.git --tag v0.1.0-alpha.6 --locked --package chattybara-cli --bin chattybara
```
...
```sh
CARGO_HOME=$PWD/.cargo-home scripts/build-release-asset.sh 0.1.0-alpha.6
```

The script writes:

- `dist/chattybara-0.1.0-alpha.6-aarch64-apple-darwin.tar.gz`
- `dist/chattybara-0.1.0-alpha.6-aarch64-apple-darwin.tar.gz.sha256`
